...
Usage:
    ...
    python referral_discover.py houston --from 5   # Resume from step 5
...
STEPS = {
    ...
    3: ("Enrich emails", "3_enrich_emails"),
    4: ("Enrich phones", "4_enrich_phones"),
    5: ("Classify + score", "5_classify_score"),
    6: ("Deduplicate", "6_dedup"),
    7: ("Export", "7_export"),
}
...
def main():
    ...
    parser.add_argument("--step", type=int, help="Run a specific step only (1-7)")
    ...
    if args.list_metros:
        ...
        for key, metro in METROS.items():
            print(f"  {key}: {metro['city']}, {metro['state']} ({metro['county']})")
        return
    ...
    if args.step:
        if args.step not in STEPS:
            print(f"Error: Step must be 1-7, got {args.step}")
            sys.exit(1)
